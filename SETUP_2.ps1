...
$tempDirName = "temp"
$scriptPath = "\\CA1MDLIDATA2408\D$\RISHIN\framework_for_uni_py\SETUP_WORK\sample_backup.py"
...
$venvDirName="venv"
$fileAPath = "\\CA1MDLIDATA2408\D$\RISHIN\framework_for_uni_py\SETUP_WORK\machine_list.txt"  # Replace with actual path to fileA
...
$destinationPath = "D$\RISHIN\framework_for_uni_py\temp\copied_path"
$destinationPath2 = "D$\RISHIN\framework_for_uni_py\temp\copied_path_inp"
...
function Copy-FileAndFolderB {
    ...
    foreach ($machine in $Machines) {
        try {
            ...
            $remoteFileA = "$remoteMachinePath\machine_list.txt"
            $remoteFolderB = "$remoteMachinePath\entirefolderB"
            
            Write-Log "Copying $FileAPath to $remoteFileA"
            Copy-Item -Path $FileAPath -Destination $remoteFileA -Force
...
function Distribute-CSVRows {
    ...
    try {
        ...
        if (!(Test-Path -Path $CsvPath)) {
            Write-Log "Error: CSV file $CsvPath not found"
            return $false
        }
        ...
        foreach ($machine in $Machines) {
            ...
            if ($machineRows.Count -eq 0) {
                Write-Log "No data rows to distribute to $machine"
                continue
            }
                ...
                $result = Invoke-Command -ComputerName $machine -ScriptBlock { 
                    ...
                    try {
                        ...
                        if (!(Test-Path $csvPath)) {
                            return "ERROR: CSV data not found at $csvPath"
                        }
...
try {
    ...
    if (-not $csvDistributionResult) {
        Write-Log "Warning: Failed to distribute CSV data, but continuing with execution"
    }

    # Copy FileA and FolderB to each machine
    Write-Log "Copying FileA and FolderB to destination path on machines..."
    Copy-FileAndFolderB -Machines $machines -FileAPath $fileAPath -FolderBPath $folderBPath -DestinationPath $destinationPath
