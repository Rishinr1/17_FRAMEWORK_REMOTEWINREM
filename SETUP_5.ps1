# Load configuration
$configFilePath = "\\CA1MDLIDATA2408\D$\RISHIN\framework_for_uni_py\SETUP_WORK\config.ini"
$config = @{}
$currentSection = ""
...
try {
    ...
    try {
        ...
        # Validate that both localTempDir and venvDirName are not null or empty
        ...
        if ([string]::IsNullOrWhiteSpace($venvDirName)) {
            Write-Log "Error: Virtual Environment Directory Name is null or empty"
            throw "Virtual Environment Directory Name is not defined"
        }

        # Use Join-Path to construct the full virtual environment path
        $venvPath = Join-Path -Path $localTempDir -ChildPath $venvDirName

        # Additional comprehensive logging
        Write-Log "Attempting to remove virtual environment"
        Write-Log "Base Directory: '$localTempDir'"
        Write-Log "VenvDir Name: '$venvDirName'"
        Write-Log "Full VenvPath: '$venvPath'"

        # Try multiple potential paths
        $potentialPaths = @(
            $venvPath,
            (Join-Path -Path $localTempDir -ChildPath "env"),
            (Join-Path -Path $localTempDir -ChildPath ".venv")
        )

        # Find existing paths and get the first one
        $foundPaths = $potentialPaths | Where-Object { Test-Path -Path $_ }

        if ($foundPaths) {
            # Take the first existing path
            $foundPath = $foundPaths | Select-Object -First 1
            
            Write-Log "Virtual environment path found: '$foundPath'"
            
            # Call cleanup function with the found path
            $cleanupResult = Remove-PythonVirtualEnvironment -Machines $machines -VenvPath $foundPath
            
            # Additional logging based on cleanup result
            if ($cleanupResult -eq $false) {
                Write-Log "Virtual environment removal encountered issues"
            }
        }
        else {
            Write-Log "No virtual environment found in the following locations:"
            $potentialPaths | ForEach-Object { Write-Log "Checked path: '$_'" }
            
            Write-Log "Listing directory contents for further investigation:"
            Get-ChildItem -Path $localTempDir | ForEach-Object { 
                Write-Log "Found item: $($_.FullName)"
            }
        }
    }
    catch {
        Write-Log "Critical Error during virtual environment cleanup"
        Write-Log "Error Message: $($_.Exception.Message)"
        Write-Log "Exception Details: $($_.ScriptStackTrace)"
    }
    finally {
        Write-Log "Virtual environment cleanup process completed"
    }
}